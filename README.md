...
- 공개 수업 목록, 검색, 상세 페이지
- 학생 회원가입과 이메일 로그인
- 튜터 지원, 증명서 비공개 업로드, 승인 대기/승인/반려 상태
- 학생 또는 승인 튜터와 관리자 사이의 1:1 수업 문의
- 관리자 수업 CRUD·공개 관리, 튜터 심사, 전체 문의 답변
- 관리자가 기획한 수업을 홈 ‘지금 만나볼 수 있는 수업’ 추천 영역에 선택 노출
- Zod 서버 검증, 역할 기반 서버 권한 검사, Supabase RLS
- 비공개 증명서 버킷과 관리자 전용 60초 서명 URL
- 한국어 이용약관·개인정보처리방침·튜터 검증 안내 초안
...
- `tutor-certificates` 비공개 버킷, 10MB 제한, PDF/JPEG/PNG MIME 제한
- 튜터 본인 업로드와 관리자 전용 열람 정책
...
`SUPABASE_SERVICE_ROLE_KEY`는 튜터 지원 직후 이메일 인증 상태와 무관하게 증명서와 지원서를 원자적으로 보관하고, 관리자 서명 URL을 생성하는 서버 전용 키입니다. 브라우저에 노출하거나 `NEXT_PUBLIC_` 접두사를 붙이지 마세요.
