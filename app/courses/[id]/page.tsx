...
export default async function CourseDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <div className="bg-slate-50/70 pb-20">
    <div className="bg-gradient-to-br from-navy-950 via-navy-900 to-blue-900 text-white"><div className="container-page grid gap-10 py-14 lg:grid-cols-[1fr_360px] lg:py-20"><div><Badge className="bg-white/10 text-blue-100">{course.subject}</Badge><h1 className="mt-5 max-w-3xl text-4xl font-black leading-tight text-white sm:text-5xl">{course.title}</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">{course.short_description}</p><div className="mt-8 flex flex-wrap gap-4 text-sm text-slate-200"><span className="flex items-center gap-2"><BookOpen className="size-4 text-blue-300" />{course.grade_level}</span><span className="flex items-center gap-2"><CalendarDays className="size-4 text-blue-300" />{course.schedule_text}</span></div>{canManageCourse && <div className="mt-8"><LinkButton href={`/admin/courses/${course.id}/edit`} variant="secondary"><Edit3 className="mr-2 size-4" />이 수업 수정하기</LinkButton></div>}</div><div className="hidden rounded-3xl bg-white/8 p-7 backdrop-blur lg:block"><ShieldCheck className="size-9 text-blue-300" /><p className="mt-5 text-xl font-bold">안심 상담</p><p className="mt-2 text-sm leading-6 text-slate-300">개인 연락처를 공개하지 않고 클래스모아 관리자와 수업에 관해 상담할 수 있어요.</p></div></div></div>
